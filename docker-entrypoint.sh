...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	...
	if [ ! -e kickstart.php ] ; then
		# if the kickstart script exists and Joomla doesn't appear to be installed AND the permissions of it are root:root, let's chown it (likely a Docker-created directory)
	...
	echo >&2 "========================================================================"
	echo >&2
	echo >&2 "This server is now configured to Kickstart a backup of Joomla!"
	echo >&2
	echo >&2 "NOTE: You will need to enter the database server address, database name,"
	echo >&2 "and database user credentials to restore the backup."
	echo >&2
	echo >&2 "Server: db"
	echo >&2 "Database: joomla"
	echo >&2 "User: joomla"
	echo >&2 "Password: joomla"
	echo >&2
	echo >&2 "========================================================================"
fi
